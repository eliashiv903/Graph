...
class Graph_AlgoTest {
	...
	public void testShortestPathDistAndShortestPathFullofNodesAndRoads() {
		...
		assertEquals(g.shortestPath(3, 2000).size()!=0, true);
	}
	...
	@Test
	void testGraph_AlgoAndDGraphFromSaveFileGui() {
		graph_algorithms f=new Graph_Algo();
		f.init("graph0.txt");
		assertEquals(f.isConnected(), false);
		assertEquals(f.shortestPathDist(0, 31),706.3782779613139);
		ArrayList<node_data> q=new ArrayList<node_data>();
		DGraph b= new DGraph(f.copy());
		q.add(b.getNode(0));
		q.add(b.getNode(19));
		q.add(b.getNode(24));
		q.add(b.getNode(31));
		ArrayList<node_data> c=new ArrayList<node_data>(f.shortestPath(0, 31));
		assertEquals(c.get(0).getKey(),q.get(0).getKey());
		assertEquals(c.get(1).getKey(),q.get(1).getKey());
		assertEquals(c.get(2).getKey(),q.get(2).getKey());
		assertEquals(c.get(3).getKey(),q.get(3).getKey());
		b.connect(30, 18, 300);
		b.connect(18, 6, 49);
		b.connect(6, 0, 60);
		graph_algorithms s=new Graph_Algo();
		s.init(b);
		...
		tar.add(31);
		tar.add(18);
		c=new ArrayList<node_data>(s.TSP(tar));
		assertEquals(c.get(0).getKey(),9);
		assertEquals(c.get(1).getKey(),10);
		assertEquals(c.get(2).getKey(),11);
		assertEquals(c.get(3).getKey(),12);
		assertEquals(c.get(4).getKey(),26);
		assertEquals(c.get(5).getKey(),31);
		assertEquals(c.get(6).getKey(),30);
		assertEquals(c.get(7).getKey(),18);
		assertEquals(c.get(8).getKey(),6);
		assertEquals(c.get(9).getKey(),0);
	}
	@SuppressWarnings("deprecation")
